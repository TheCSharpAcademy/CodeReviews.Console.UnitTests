...
namespace CodingTracker.Arashi256.Views
{
    internal class CodingReportsView
    {
        ...
        public void CodingSessionReports()
        {
            ...
            {
                Console.WriteLine("\n");
                AnsiConsole.Write(new Text("R E P O R T S   M E N U").Centered());
        ...
        private void ProcessReportMenu(int option)
        {
            ...
            {
                AnsiConsole.Markup("[red]Error: Session controller is not initialized.[/]\n");
                return;
            }
        ...
        private void DisplayCodingSessionsBetweenDates()
        {
            ...
            // Check that sessionsDateTimes is not null and contains two non-null values
            if (sessionsDateTimes != null && sessionsDateTimes.Length == 2 && sessionsDateTimes[0].HasValue && sessionsDateTimes[1].HasValue)
            {
                DisplayCodingSessions(_sessionController.GetCodingSessionsBetweenDates(sessionsDateTimes[0]!.Value, sessionsDateTimes[1]!.Value, sortOrder));
                CommonUI.Pause();
            }
            else
            {
                AnsiConsole.Markup("[red]Invalid date range. Please try again.[/]\n");
            }
        }
