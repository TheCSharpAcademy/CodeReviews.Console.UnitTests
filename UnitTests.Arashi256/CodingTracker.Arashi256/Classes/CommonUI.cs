...
using static CodingTracker.Arashi256.Classes.Validator;
    ...
    public class CommonUI
    {
        ...
        public static DateTime? GetUserDateTimeDialog()
        {
            ...
            while (!dateTime.HasValue)
            {
                ...
                var userInput = AnsiConsole.Prompt(new TextPrompt<string>("Enter a date/time in the format 'dd-MM-yy HH:mm:ss':").PromptStyle("white"));
                if (userInput.ToLower() == "q")
                {
                    return null;
                }
                // Validate date
                if (validator.TryParseDate(userInput, out DateTime result))
                {
                    dateTime = result;
                }
                else
