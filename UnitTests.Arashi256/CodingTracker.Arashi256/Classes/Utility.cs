...
namespace CodingTracker.Arashi256.Classes
{
    public class Utility
    {
        ...
        {
            if (codingSessions == null || codingSessions.Length == 0)
            {
                return "00:00:00"; // Return "00:00:00" if there are no sessions
            }
            TimeSpan totalDuration = TimeSpan.Zero;
            foreach (CodingSession codingSession in codingSessions)
            {
                if (TimeSpan.TryParse(codingSession.Duration, out TimeSpan duration))
                {
                    totalDuration += duration;
                }
                else
                {
                    // Handle invalid duration formats if necessary, e.g., skip or log
                    Console.WriteLine($"Invalid duration format: {codingSession.Duration}");
                }
            }
        ...
        public static string AverageCodingSessions(CodingSession[] sessions)
        {
            ...
            if (validSessionsCount == 0) return "00:00:00"; // Return "00:00:00" if no valid sessions
            double averageTicks = totalDuration.Ticks / (double)validSessionsCount;
