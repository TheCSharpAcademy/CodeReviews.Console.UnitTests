...
internal static class UserInterface
{
    ...
    private static void SetGoal()
    {
        ...
        do
        {
            ...
            isValidEntry = Validator.IsValidHour(input).Item1; 
            codingHours= Validator.IsValidHour(input).Item2;
            if (!isValidEntry)
    ...
    private static DateTime[] GetDateInputs()
    {
        ...
        DateTime endDate;
        while (!Validator.IsValidDate(endDateInput).Item1)
        {
            endDateInput = AnsiConsole.Ask<string>("\n\nInvalid date. Format: dd-mm-yy hh:mm (24 hour clock). PLease try again\n\n");
        }
        endDate = Validator.IsValidDate(endDateInput).Item2;
